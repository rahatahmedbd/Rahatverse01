...
- Ambient scientific background — very faint, slowly drifting atoms, molecules,
  DNA strands, planets and math/physics formulas (E=mc², π, Σ, a²+b²=c²…) fixed
  behind all content. Pure decoration (pointer-events-none, aria-hidden),
  reduced to ~55% opacity in light theme and fully static with
  `prefers-reduced-motion`, so the page stays calm and readable.
